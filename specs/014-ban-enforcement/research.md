...
- **JWT Metadata**: Storing `is_active` in Supabase `user_metadata`. 
  - *Rejected*: Requires manual metadata sync on every ban/unban; JWTs are harder to invalidate immediately than a side-cookie.
- **In-Memory Cache (Redis)**: Using Redis to store ban status.
  - *Rejected*: Adds infrastructure complexity and latency compared to a simple cookie-based cache for a basic boolean check.
